...
-- This migration creates a complete glossary system from scratch
-- It includes table definition, functions, triggers, indexes, and policies

-- Drop existing table if it exists (comment out if you want to preserve existing data)
-- DROP TABLE IF EXISTS public.glossaries;

-- Create glossaries table with all required fields
...
-- Add glossary_json column to series table
...
-- Create triggers for glossary changes
...
-- Create utility functions for glossary management
...
BEGIN
  ...
  FROM 
    public.glossaries g
  WHERE 
    g.series_id = search_glossary_terms.series_id
    AND (
...
CREATE OR REPLACE FUNCTION public.get_glossary_stats(series_id UUID)
RETURNS TABLE (
  term_type TEXT,
  count BIGINT
) AS $$
BEGIN
  ...
  FROM 
    public.glossaries g
  WHERE 
    g.series_id = get_glossary_stats.series_id
  GROUP BY 
...
BEGIN
  ...
  PERFORM public.update_series_glossary_json(series_id);
...
-- Update all existing series with the glossary data
DO $$
DECLARE
  series_rec RECORD;
BEGIN
